...
public class KnowOrderAdapter extends RecyclerView.Adapter<KnowOrderAdapter.ViewHolder> implements View.OnClickListener {
    int type = 2;//1全部司机 2我的司机 3指定司机
    Context context;
    ...
    public KnowOrderAdapter(List<OrderNew> orderList, AppData app,
    ...
    @Override
    public KnowOrderAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        context=parent.getContext();
        View view = LayoutInflater.from(context).inflate(R.layout.item_order_know, parent, false);
    ...
    @Override
    public void onBindViewHolder(final KnowOrderAdapter.ViewHolder holder, int position) {
        OrderNew order = orderList.get(position);
        setValue(order, holder);
        setBtnState(order, holder);
    }

    private void setValue(final OrderNew order, KnowOrderAdapter.ViewHolder holder) {
...
//        Picasso.with(context).load(assembleImgUrl(order.getHeaderUrl())).memoryPolicy(NO_CACHE, NO_STORE).into(holder.allorder_img_head);
    }
    ...
    private void setBtnState(final OrderNew order, KnowOrderAdapter.ViewHolder holder) {
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.allorder_layout_centercontent:
                OrderDetailsActivity.start(context, order.getId(), 2);
                break;
